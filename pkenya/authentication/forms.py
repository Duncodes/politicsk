...
from pkenya.settings import ALLOWED_SIGNUP_DOMAINS
...
class SignUpFormPolitic(forms.ModelForm):
    username = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'form-control'}),
        max_length=30,
        required=True,
    ...
    CHOICES=(('1', 'Mp',), ('2', 'Governoer',),('3', 'Senator',),('4', 'MCA',),)
    post= forms.ChoiceField(widget=forms.Select, choices=CHOICES)

    class Meta:
        model = User
        exclude = ['last_login', 'date_joined']
        fields = ['username', 'email', 'password','post','confirm_password' ]


    def __init__(self, *args, **kwargs):
        super(SignUpFormPolitic, self).__init__(*args, **kwargs)
        self.fields['username'].validators.append(ForbiddenUsernamesValidator)
        self.fields['username'].validators.append(InvalidUsernameValidator)
        self.fields['username'].validators.append(
            UniqueUsernameIgnoreCaseValidator)
    ...
    def clean(self):
        super(SignUpFormPolitic, self).clean()
        password = self.cleaned_data.get('password')
        confirm_password = self.cleaned_data.get('confirm_password')
        if password and password != confirm_password:
            self._errors['password'] = self.error_class(
